...
class MoodMusicApp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        title_bar.setStyleSheet("background-color: #16213e;")
        from PyQt5.QtWidgets import QHBoxLayout
        tl = QHBoxLayout(title_bar)
        title_lbl = QLabel("🎵 MOOD MUSIC PLAYER - by Ronit Gulia")
        title_lbl.setFont(QFont("Courier", 14, QFont.Bold))
        ...
        # LEFT: Camera
        left_frame = QFrame()
        left_frame.setStyleSheet("background-color: #16213e; border-radius: 4px;")
        left_layout = QVBoxLayout(left_frame)
        cam_hdr = QLabel("📷 LIVE FEED")
        cam_hdr.setFont(QFont("Courier", 9, QFont.Bold))
        cam_hdr.setStyleSheet("color: #555; padding: 4px;")
        left_layout.addWidget(cam_hdr)
        self.cam_label = QLabel()
        self.cam_label.setFixedSize(480, 360)
        self.cam_label.setStyleSheet("background-color: #000;")
        ...
        mid_frame.setStyleSheet("background-color: #16213e; border-radius: 4px;")
        mid_layout = QVBoxLayout(mid_frame)
        bar_hdr = QLabel("📊 EMOTION SCORES")
        bar_hdr.setFont(QFont("Courier", 9, QFont.Bold))
        bar_hdr.setStyleSheet("color: #555; padding: 4px;")
        ...
        self.emotion_lbl.setFont(QFont("Courier", 18, QFont.Bold))
        self.emotion_lbl.setStyleSheet("color: #FFD700;")
        self.emotion_lbl.setAlignment(Qt.AlignCenter)
        mid_layout.addWidget(self.emotion_lbl)
        self.conf_lbl = QLabel("Confidence: —")
        self.conf_lbl.setFont(QFont("Courier", 9))
        ...
        self.status_lbl.setFont(QFont("Courier", 8))
        ...
        self.voice_cb.setStyleSheet("color: #888; font-family: Courier; font-size: 9pt;")
        ...
        # RIGHT: History
        right_frame = QFrame()
        right_frame.setStyleSheet("background-color: #16213e; border-radius: 4px;")
        right_layout = QVBoxLayout(right_frame)
        hist_hdr = QLabel("📈 MOOD HISTORY")
        hist_hdr.setFont(QFont("Courier", 9, QFont.Bold))
        hist_hdr.setStyleSheet("color: #555; padding: 4px;")
        ...
        bot.setStyleSheet("background-color: #0f3460;")
        bot_layout = QHBoxLayout(bot)
        bot_layout.setContentsMargins(15, 0, 15, 0)
        self.song_lbl = QLabel("🎵 No song playing yet")
        self.song_lbl.setFont(QFont("Courier", 9))
        self.song_lbl.setStyleSheet("color: #aaa;")
        bot_layout.addWidget(self.song_lbl)
        bot_layout.addStretch()
        self.cd_lbl = QLabel("")
        self.cd_lbl.setFont(QFont("Courier", 9))
        self.cd_lbl.setStyleSheet("color: #e94560;")
        bot_layout.addWidget(self.cd_lbl)
        quit_btn = QPushButton(" QUIT ")
        quit_btn.setFont(QFont("Courier", 9, QFont.Bold))
